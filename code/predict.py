...
from lineage import fingerprint, record, split_id_of
...
def predict(features_path, model_path=None, labels_path=None, out_path=None,
    ...
    model_path = resolve_model(run_id, model_path)

    print(f"\nScoring {features_path.name} with "
    ...
    if meta:
        if meta.get("feature_version") != FEATURE_VERSION:
            raise ValueError(
                f"this model was trained on feature_version "
                f"{meta.get('feature_version')}, config.py is now at {FEATURE_VERSION} "
                f"-- retrain, or check out the matching config")

        # the model learned one cut of the data; scoring another cut's holdout
        # would quietly report a number that means nothing
        model_split, data_split = meta.get("split_id"), split_id_of(features_path)
        if model_split and data_split and model_split != data_split:
            raise ValueError(
                f"model was trained on split {model_split} but "
                f"{features_path.name} comes from split {data_split} -- these "
                f"are different cuts of the data and cannot be compared")
        if meta.get("run_id"):
            score = meta.get("valid_amex", {}).get("score")
            print(f"  run {meta['run_id']}, split {meta.get('split_id')}"
                  + (f", validation score {score:.6f}" if score else ""))
    ...
    if out_path:
        ...
        record(out_path, source=fingerprint(features_path),
               split_id=split_id_of(features_path),
               run_id=(meta or {}).get("run_id"), produced_by="predict.py",
               customers=len(pred))
...
def main():
    ...
    ap.add_argument("--model", default=None, help="a booster file to use directly")
    ...
    predict(features, args.model, labels, out, args.run)
